...
func (d *documentRepo) GetDocsWithDms(ctx context.Context, offset uint, size uint, sortBy string) ([]*biz.Document, map[uint][]*biz.Dimension, error) {
	if offset < 0 || size <= 0 {
	...
	if err := d.data.db.Model(&biz.Document{}).Select("id,created_at,updated_at,uid,type,name,intro,title,download_num,scan_num,like_num,is_load_search,is_save").Order(sortBy + " desc,created_at desc").Limit(int(size)).Offset(int(offset)).Find(&docs).Error; err != nil {
		return nil, nil, err
	}
...
func (d *documentRepo) GetDocsByDidWithDms(ctx context.Context, did uint, offset uint, size uint, sortBy string) ([]*biz.Document, map[uint][]*biz.Dimension, error) {
	if did <= 0 || offset < 0 || size <= 0 {
	...
	docs := make([]*biz.Document, 0)
	subQuery := d.data.db.Select("doc_id").Where("did = ?", did).Table("doc_with_dm")
	if err := d.data.db.Model(&biz.Document{}).Select("id,created_at,updated_at,uid,type,name,intro,title,download_num,scan_num,like_num,is_load_search,is_save").Where("id in (?)", subQuery).Order(sortBy + " desc,created_at desc").Limit(1).Limit(int(size)).Offset(int(offset)).Find(&docs).Error; err != nil {
		return nil, nil, err
	}
