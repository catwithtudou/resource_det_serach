...
type IDocumentRepo interface {
	...
	GetDocsByDidWithDms(ctx context.Context, did uint, offset uint, size uint) ([]*Document, map[uint][]*Dimension, error)
...
type IDocumentUsecase interface {
	...
	GetPartDocs(ctx context.Context, did uint, offset uint, size uint) ([]*Document, map[uint]map[string][]*Dimension, error)
}
