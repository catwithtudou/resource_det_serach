...
type IDocumentRepo interface {
	...
	GetDocsWithDms(ctx context.Context) ([]*Document, map[uint][]*Dimension, error)
...
type IDocumentUsecase interface {
	...
	GetAllDocs(ctx context.Context) ([]*Document, map[uint]map[string][]*Dimension, error)
	...
	GetPartDocs(ctx context.Context, did uint) ([]*Document, map[uint]map[string][]*Dimension, error)
}
