...
func (c *classDocumentUsecase) SearchAllQuery(ctx context.Context, queryStr string, partId uint) ([]*biz.ClassDocument, error) {
	if queryStr == "" {
	...
	res, err := c.repo.SearchQueryByPart(ctx, queryStr, dmInfo.Name)
	if err != nil {
